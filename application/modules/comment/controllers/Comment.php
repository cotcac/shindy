...
class Comment extends MX_Controller {
    ...
    function edit(){
      ...
      function edit_validation(){
        ...
        $this->form_validation->set_rules('comment_content', 'Comment', 'required|trim|min_length[1]|max_length[500]');
        $this->form_validation->set_rules('comment_id', 'ID', 'required');
        
        if ($this->form_validation->run($this) == TRUE) {
            $update_id= $this->input->post('comment_id', TRUE);
            $data['comment_content'] = $this->input->post('comment_content', TRUE);
            $data['comment_id'] = $update_id;
            $this->_update($update_id, $data);
            $quotation_id=$this->reuse_model_function->_detail('comment','comment_id',$update_id)->fk_question_id;
            //echo $quotation_id;
    ...
    function index($question_id=21){
        $id=$question_id;//change this ID
        $data['id']=$question_id;//change this ID
        $this->load->model('mdl_comment');
          if ($this->session->userdata('logged_in') == TRUE) {
          $user_id= $this->session->user_id;
          }else{
