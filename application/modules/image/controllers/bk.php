...
class Image extends MX_Controller {
    ...
    function docrop() {
        ...
        $this->create_thumb($src);
        // display the  photo - "?time()" to force refresh by the browser
           $response = array();//big array
          $response['success'] = true;
          $response['image'] ='<img src="'.base_url().'img/upload/'. $img_name.'?'.time().'">';
          $response['thumb'] ='img/upload/'.$img_name;
          echo json_encode($response);
       // echo '<img src="' . base_url() . 'img/upload/' . $img_name . '?' . time() . '">';
    }
    // same with docrop only different location img/upload->img/avatar
    function docrop_avatar() {
        ...
        imagejpeg($dst_r, $src, $jpeg_quality);
        // update user avatar
        $this->update_user_avatar($this->session->user_id, 'img/avatar/'.$img_name);
